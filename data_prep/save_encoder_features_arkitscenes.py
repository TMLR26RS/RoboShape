"""
save_encoder_features_arkitscenes.py

For every scene in --data_dir (ARKitScenes Validation):
  ...
  Task 3 – Bed scene-type tensor 
      For each coarsest voxel: 1 if its scene contains a "bed" in PREDICTED labels, else 0
...
def process_scene(scene_path, model, seg_head, transform, softmax, min_voxels, max_voxels, max_input_points, non_bed_keep_ratio):
    ...
    Returns (feat_np, wall_mask, is_bed_scene) or None on failure.
      ...
      is_bed_scene : bool               — True if bed is present in predictions
    """
    # ── Load & transform ──────────────────────────────────────────────────────
    try:
    ...
    try:
        with torch.inference_mode():
            ...
            # ── Task 3: bed scene type from predictions (Option A with threshold) ────────────
            bed_voxel_count = np.sum(voxel_pred == BED_IDX)
            is_bed_scene = bool(bed_voxel_count > (0.03 * N_coarsest))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir",   type=str,
                        default="/path/to/dataset/arkitscenes/Validation",
                        help="Directory containing scene folders")
    parser.add_argument("--output_dir", type=str,
                        default="/path/to/dataset/arkitscenes_tensors",
                        help="Directory to save output tensors")
    parser.add_argument("--json_log",   type=str,
                        default="/path/to/dataset/arkitscenes_tensors/scene_log.json",
    ...
    # ARKitScenes are also arbitrary IDs (numeric strings)
